...
def vessel_details_page():
    ...
    with st.container():
        ...
        with cols[1]:
            ...
            # Safe datetime input
            cosp = st.session_state.voyage_data['cosp']
            eosp = st.session_state.voyage_data['eosp']
            cosp_date = st.date_input("COSP Date", value=cosp.date(), key="cosp_date_input")
            cosp_time = st.time_input("COSP Time", value=cosp.time(), key="cosp_time_input")
            st.session_state.voyage_data['cosp'] = datetime.combine(cosp_date, cosp_time)

            eosp_date = st.date_input("EOSP Date", value=eosp.date(), key="eosp_date_input")
            eosp_time = st.time_input("EOSP Time", value=eosp.time(), key="eosp_time_input")
            st.session_state.voyage_data['eosp'] = datetime.combine(eosp_date, eosp_time)
        
        with cols[2]:
            st.subheader("CP Parameters")
            st.session_state.cp_params['warranted_speed'] = st.number_input("Warranted Speed (knots)", 13.0, 0.0, step=0.1)
            st.session_state.cp_params['warranted_consumption'] = st.number_input("Warranted Consumption (MT/day)", 19.9, 0.0, step=0.1)
            st.session_state.cp_params['fuel_tolerance'] = st.number_input("Fuel Tolerance (%)", 5.0, 0.0, 100.0)
            st.session_state.cp_params['speed_tolerance'] = st.number_input("Speed Tolerance (knots)", 0.5, 0.0, step=0.1)
    ...
    cols = st.columns(2)
    with cols[0]:
        st.session_state.weather_def['beaufort'] = st.selectbox(
            "Max Beaufort Scale for Good Weather",
            options=[(n, f"Beaufort {n}") for n in range(13)],
            format_func=lambda x: x[1]
        )
    with cols[1]:
        wave_heights = [round(i * 0.25, 2) for i in range(21)]
        st.session_state.weather_def['wave_height'] = st.selectbox("Max Significant Wave Height (m)", options=wave_heights, index=8)
...
def dashboard_page():
    ...
    if 'performance_data' in st.session_state and 'weather_data' in st.session_state:
        df = pd.merge(st.session_state.performance_data, st.session_state.weather_data, on='timestamp')

        st.subheader("Speed Analysis")
        fig1 = px.scatter(df, x='wind_speed', y='speed', color='wave_height', trendline="lowess")
...
def generate_pdf():
    st.warning("PDF generation not implemented yet.")
